...
function RouteComponent() {
  const [list, setList] = useState<{ id: number; name: string; des: string; cover: string }[]>(testData2);
  ...
  useEffect(() => {
    ...
    // 指定响应数据的类型
    _axios.post<PageListResponse>('/project/pageList/', { size: 1000 }).then((res) => {
  ...
  return (
    <div
      ...
      <div id="pre-view-top" className="sticky-top" style={{ height: '2rem', scrollBehavior: 'smooth' }}>
        —————————————————鼠标滑到下面画布—————————————————
      </div>

      <ButtonGroup size="sm">
        {list.map((item: { id: number; name: string; des: string; cover: string }) => {
          return (
            <Button
              variant={btnColor}
              key={item.id}
      ...
      <div
        ...
        {_item && <Viewer3d item={_item} callBack={callBack} callBackError={callBackError} />}
      </div>
    </div>
  );
}
