...
let scene, camera, perspectiveCamera, orthographicCamera, controls, controls1, controls2, renderer, divElement, transfControls, transfControls1, transfControls2;
function animate() {
...
function addLight() {
  const light = new DirectionalLight(16777215, 2.16);
  light.shadow.mapSize.width = 2048;
  light.shadow.mapSize.height = 2048;
  light.shadow.camera.near = 0.5;
  light.shadow.camera.far = 5e3;
...
let perspectiveCameraPosition = new Vector3(-5, 5, 8);
function setCameraType(cameraType, cameraUp) {
...
function raycasterSelect(event) {
...
let boxHelper;
function setTransformControls(selectedMesh) {
  ...
  if (boxHelper) {
    ...
    boxHelper = new BoxHelper(selectedMesh[0], 16776960);
    scene.add(boxHelper);
  }
...
function getObjectNameByName(object3D) {
  return object3D.name.trim() === "" ? object3D.type : object3D.name;
}
function onWindowResize(canvas2, camera2, renderer) {
  if (canvas2.current !== null) {
    camera2.aspect = canvas2.current.offsetWidth / canvas2.current.offsetHeight;
    camera2.updateProjectionMatrix();
    renderer.setSize(canvas2.current.offsetWidth, canvas2.current.offsetHeight);
  }
}
...
function Viewer3d({
  ...
  useEffect(() => {
    if (canvas3d.current) {
      createScene$1(canvas3d.current);
    }
    window.addEventListener(
      "resize",
      () => onWindowResize(canvas3d, getCamera$1(), getRenderer$1())
    );
    return () => {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: canvasStyle, ref: canvas3d });
}

export { addGridHelper as a, getScene as b, getObjectNameByName as c, setTransformControls as d, Viewer3d as default, setCamera as e, getDivElement as f, getCamera as g, createScene as h, getRenderer as i, jsxRuntimeExports as j, setCameraType as k, getTransfControls as l, addGlb as m, onWindowResize as o, raycasterSelect as r, setScene as s, takeScreenshot as t };
